...
const CORES = [
  { slug: 'tabby-shoulder-preto',           cor: 'Preto',            hex: '#1A1714', material: 'Couro liso',                     estoque: 5, fotos: ['01.jpg'],                           modelo: ['02.png', '03.png', '04.png'] },
  ...
  { slug: 'tabby-shoulder-branco',          cor: 'Branco',           hex: '#EDE7DD', material: 'Couro liso',                     estoque: 5, fotos: ['01.jpg', ...nums(2, 10), '11.gif'],  modelo: nums(12, 17) },
  { slug: 'tabby-shoulder-branco-off',      cor: 'Branco Off',       hex: '#E8E0D2', material: 'Couro liso',                     estoque: 3, fotos: jpgs(6),                              modelo: ['09.jpg', '07.jpg', '08.jpg', '10.jpg', '11.jpg'] },
...
/* A faixa "Você também pode gostar" tem slideshow: peça com uma foto de
   estúdio só fica sem graça no meio das que giram. Então as com 2+ fotos
   vêm primeiro; mesma categoria antes do resto; ordem do catálogo dentro
   de cada grupo. */
export function relacionados(produto, limite = 4) {
  if (!produto) return [];
  const outros = PRODUTOS.filter((p) => p.id !== produto.id);
  const peso = (p) =>
    (p.cat === produto.cat ? 0 : 2) + ((p.fotosProduto?.length ?? 0) > 1 ? 0 : 1);
  return outros
